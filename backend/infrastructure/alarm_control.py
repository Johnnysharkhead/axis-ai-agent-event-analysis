'''
This module provides the core business logic for controlling Axis speaker alarms.
It was extracted from routes/alarm.py to enable reuse by both HTTP routes and
background infrastructure components (e.g., intrusion detection).
'''
...
# ---- Public interface ----
def play_once():
    return _do_play_once()


def start_loop(interval_seconds: int = 5) -> bool:
    global _alarm_thread
    with _alarm_lock:
        if _alarm_thread and _alarm_thread.is_alive():
            return False
        _alarm_stop_event.clear()
        _alarm_thread = threading.Thread(target=_alarm_worker, args=(interval_seconds,), daemon=True)
        _alarm_thread.start()
    return True
